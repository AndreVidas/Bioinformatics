...
#### load data
genes <- read.csv(file = "dataChunqin/results/DE_gene_list/exp.csv", row.names = 1)
...
up_top10 <- head(genes[order(-genes$log2FoldChange),], n = 10)
down_top10 <- head(genes[order(genes$log2FoldChange),], n = 10)
...
ggplot(genes, aes(x = log2FoldChange, y = -log10(padj))) + xlab(expression(log[2]("FC"))) +
    ...
    geom_text_repel(
        ...
        data = rbind(up_top10, down_top10),
        aes(label = Gene),
        size = 5,
